...
/* IMPORTANT: To use the countdown timer - this script must be updated with the unique promotion ID - copy: #### and replace the one instance with the actual promo id */
/* This script implements a countdown timer for a promotional campaign. It dynamically creates a countdown bar, displays the time remaining until midnight, and adjusts its position relative to other elements on the page. The script includes helper functions for debugging, debouncing, and observing DOM changes. It also listens for specific messages to initialize the countdown bar based on a unique promotion ID. */
...
End of Countdown Display Script */
  ...
  window.addEventListener("message", (e) => {
    if(e.data.key && e.data.key == 'init' && e.data.promotion_id && e.data.promotion_id == ####) {
      createCountdownBar();
    }
  });
